...
async function saveAiRequestResponseLog(params: {
  sessionId: string;
  requestBody: unknown;
  ...
  const safeSessionId = params.sessionId.replace(/[^a-zA-Z0-9_-]/g, "_");

  const logDir = path.join(process.cwd(), "public", "ai-logs");
  ...
  let existingLogFile: AiSessionLogFile;

  try {
    const existingContent = await fs.readFile(filePath, "utf-8");

    existingLogFile = JSON.parse(existingContent) as AiSessionLogFile;
  } catch {
    existingLogFile = {
  ...
  existingLogFile.updated_at = timestamp;

  existingLogFile.logs.push({
    timestamp,
  ...
  await fs.writeFile(
    filePath,
    JSON.stringify(existingLogFile, null, 2),
    "utf-8",
  );
...
`.trim();
    ...
    console.log("AI request/response saved:", aiLog.publicUrl);
    if (!assistantContent) {
      return NextResponse.json(
        {
